module ApplicationHelper
	...
	def add_remove_link(show)
		if show.is_a?(Serial)
			if current_user && current_user.serials.include?(show)
				link_to "Remove from My Series", remove_serial_path, class: "remove-show-button", remote: true
			else
				link_to "Add to My Series", add_serial_path, class: "add-show-button", remote: true
			end
		elsif show.is_a?(Movie)
			if current_user && current_user.movies.include?(show)
				link_to "Remove from My Movies", remove_movie_path, class: "remove-show-button", remote: true
			else
				link_to "Add to My Movies", add_movie_path, class: "add-show-button", remote: true
			end
		end
	end
	...
	def watch_unwatch_link(show)
		if show.is_a?(Serial)
			if current_user && current_user.watchlist.serials.include?(show)
				link_to "Remove from My watchlist", unwatch_serial_path, class: "unwatch-show-button", remote: true
			else
				link_to "Add to My Watchlist", watch_serial_path, class: "watch-show-button", remote: true
			end
		elsif show.is_a?(Movie)
			if current_user && current_user.watchlist.movies.include?(show)
				link_to "Remove from My Watchlist", unwatch_movie_path, class: "unwatch-show-button", remote: true
			else
				link_to "Add to My Watchlist", watch_movie_path, class: "watch-show-button", remote: true
			end
		end
	end
	...
	def seen_unseen_movie_link(episode)
		if current_user && current_user.watched.movies.include?(episode)
			link_to "Mark as unseen", unseen_movie_path, class: "unwatch-episode-button", remote: true
		else
			link_to "Mark as seen", seen_movie_path, class: "watch-episode-button", remote: true
		end
